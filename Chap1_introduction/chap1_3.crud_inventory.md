...
```js
const myCursor = (restriction,projection = {_id : 0 }) => db.collection.find(restriction, projection);

/**

const myCursor = (restriction,projection = { _id : 0 }) => { 

   return  db.collection.find(restriction, projection);
}
...
### 09. Affichez le nom des sociétés et leur(s) tag(s) si et seulement si ces sociétés ont des tags ou un tag.
...
## 01 Exercice faire une augmentation de 50% 

Augmentez de 50% la quantité de chaque document qui a un status C ou D.


## 02 Exercice ajouter un champ et calculer
...
## 03 Exercice suppression d'un champ
...
## 07 Exercice calculer la somme des quantités par société
